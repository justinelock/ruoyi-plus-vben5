...
const log = ref<null | TradeContractSettlementLog>(null);
...
const sections = computed(() => {
  const d = log.value;
  if (!d) {
    return [] as { items: DescriptionsProps['items']; title: string; }[];
  }

  const blocks: { items: DescriptionsProps['items']; title: string; }[] = [];
