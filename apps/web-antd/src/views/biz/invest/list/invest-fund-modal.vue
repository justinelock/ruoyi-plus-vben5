...
async function applyDetail(detail: InvestList) {
  ...
  await formApi.setValues({
    ...
    rateMinPct:
      detail.rateMin === null || detail.rateMin === undefined
        ? undefined
        : decimalToPercent(detail.rateMin),
    rateMaxPct:
      detail.rateMax === null || detail.rateMax === undefined
        ? undefined
        : decimalToPercent(detail.rateMax),
    ratePct:
      detail.rate === null || detail.rate === undefined
        ? undefined
        : decimalToPercent(detail.rate),
...
async function handleSubmit() {
  ...
  try {
    await (editingId.value
      ? investListUpdate({ ...payload, id: editingId.value })
      : investListAdd(payload));
    emit('reload');
    modalApi.close();
  } catch (error) {
    console.error(error);
